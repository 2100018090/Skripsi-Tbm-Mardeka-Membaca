@extends('admin.layouts.autentikasi.login')
@section('content')
@vite(['resources/css/app.css','resources/js/app.js'])
    ...
    <div class="flex flex-col items-center justify-center px-6 pt-8 mx-auto md:h-screen pt:mt-0">
        <a href="{{ url('/admin/login') }}"
            ...
            <img src="{{ asset('storage/icon/logo.svg') }}" class="mr-4 h-11" alt="Logo">
        </a>
        <!-- Card -->
        <div class="w-full max-w-xl p-6 space-y-8 sm:p-8 bg-white rounded-lg shadow dark:bg-gray-800">
            ...
            <form class="mt-8 space-y-6" method="POST" action="{{ route('prosesLogin') }}">
                @csrf
                <div>
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Email Atau
                        Username</label>
                    <input type="email" name="email" id="email"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="masukkan email atau password" required>
                </div>
                <div>
                    <label for="password"
